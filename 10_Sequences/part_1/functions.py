...
def train_and_eval(lang, train_loader, dev_loader, test_loader, bidirectional=False, dropout=False, PAD_TOKEN = 0):
    ...
    for x in tqdm(range(0, runs)):
        model = ModelIAS(hid_size, out_slot, out_int, emb_size, vocab_len,
                        bidirectional=bidirectional, dropout=dropout, pad_index=PAD_TOKEN).to(device)
    ...
    save_model(model, bidirectional, dropout)